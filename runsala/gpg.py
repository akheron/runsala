...
def gpg_encrypt_string(config, passphrase, content):
    with tempfile.TemporaryFile() as stderr:
        ...
        gnupg.run(
            ['--symmetric'],
            create_fhs=['stdin', 'passphrase', 'stdout'],
            attach_fhs={'stderr': stderr})
        ...
        result = gnupg.handles['stdout'].read()
        gnupg.handles['stdout'].close()
...
def gpg_decrypt_string(string, passphrase):
    with tempfile.TemporaryFile() as stderr:
        gnupg = GnuPG(['--armor'])
        ...
        gnupg.handles['stdin'].write(string)
        gnupg.handles['stdin'].close()
